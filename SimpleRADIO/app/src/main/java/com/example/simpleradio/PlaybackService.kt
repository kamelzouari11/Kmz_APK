...
class PlaybackService : MediaSessionService() {
    ...
    override fun onCreate() {
        ...
        if (powerManager != null) {
            wakeLock = powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "SimpleRADIO:WakeLock")
        }
            ...
            .setCallback(object : MediaSession.Callback {
                override fun onConnect(
                    ...
                    val availableSessionCommands = connectionResult.availableSessionCommands.buildUpon()
                        .add(androidx.media3.session.SessionCommand("UPDATE_METADATA", Bundle.EMPTY))
                        .build()
                ...
                override fun onCustomCommand(
                    ...
                    if (customCommand.customAction == "UPDATE_METADATA") {
                        ...
                        serviceScope.launch {
                            overrideArtworkUri = artworkUrl
                            if (artworkUrl != null) {
                                try {
                                    val url = java.net.URL(artworkUrl)
                                    overrideArtworkData = url.readBytes()
                                } catch (e: Exception) { overrideArtworkData = null }
                            } else {
                                overrideArtworkData = null
                            }
                            
                            withContext(Dispatchers.Main) {
                                customPlayer?.notifyMetadataChanged()
                            }
                        }
        ...
        setMediaNotificationProvider(object : androidx.media3.session.DefaultMediaNotificationProvider(this) {
             override fun getMediaButtons(
                ...
                val playPauseButton = if (session.player.isPlaying) {
                     androidx.media3.session.CommandButton.Builder().setPlayerCommand(androidx.media3.common.Player.COMMAND_PLAY_PAUSE).setIconResId(android.R.drawable.ic_media_pause).setDisplayName("Pause").build()
                } else {
                     androidx.media3.session.CommandButton.Builder().setPlayerCommand(androidx.media3.common.Player.COMMAND_PLAY_PAUSE).setIconResId(android.R.drawable.ic_media_play).setDisplayName("Play").build()
                }
                val prevButton = androidx.media3.session.CommandButton.Builder().setPlayerCommand(8).setIconResId(android.R.drawable.ic_media_previous).setDisplayName("Previous").setEnabled(true).build()
                val nextButton = androidx.media3.session.CommandButton.Builder().setPlayerCommand(9).setIconResId(android.R.drawable.ic_media_next).setDisplayName("Next").setEnabled(true).build()
                return com.google.common.collect.ImmutableList.of(prevButton, playPauseButton, nextButton)
             }
        })
    ...
    inner class CustomForwardingPlayer(player: androidx.media3.common.Player) : androidx.media3.common.ForwardingPlayer(player) {
        ...
        override fun getMediaMetadata(): androidx.media3.common.MediaMetadata {
            ...
            if (overrideArtworkUri != null) try { builder.setArtworkUri(android.net.Uri.parse(overrideArtworkUri)) } catch(e:Exception){}
            if (overrideArtworkData != null) builder.setArtworkData(overrideArtworkData, androidx.media3.common.MediaMetadata.PICTURE_TYPE_FRONT_COVER)
