...
class SyncService(private val dao: IptvDao) {
    ...
    private suspend fun refreshLiveXtream(profile: ProfileEntity) {
        ...
        val categoryEntities = apiCategories.mapIndexed { index, cat ->
            CategoryEntity(cat.category_id ?: "0", cat.category_name, profile.id, type = "LIVE", sortOrder = index)
        }
        ...
        dao.syncProfileData(profile.id, categoryEntities, channelEntitiesMap.values.toList(), linkEntities, "LIVE")
    }
    ...
    private suspend fun refreshVodXtream(profile: ProfileEntity) {
        try {
            ...
            dao.syncProfileData(profile.id, categoryEntities, channelEntitiesMap.values.toList(), linkEntities, "VOD")
        } catch (e: Exception) {
    ...
    private suspend fun refreshStalker(profile: ProfileEntity) {
        ...
        val categoryEntities = genresResponse.js.mapIndexed { index, genre ->
            CategoryEntity(genre.id, genre.title, profile.id, type = "LIVE", sortOrder = index)
        }
        ...
        channelsData.forEachIndexed { index, chMap ->
            ...
            val icon = if (!logoUrl.isNullOrEmpty()) {
        ...
        dao.syncProfileData(profile.id, categoryEntities, channelEntitiesMap.values.toList(), linkEntities, type = "LIVE")
    }
}
