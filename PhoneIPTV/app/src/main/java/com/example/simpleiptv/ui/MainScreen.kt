    ...
    BackHandler(enabled = true) {
        when {
            viewModel.isFullScreenPlayer -> {
                // Retour à la liste depuis le player sans arrêter la lecture
                viewModel.isFullScreenPlayer = false
            }
            viewModel.isShowingChannelsPortrait -> {
                viewModel.isShowingChannelsPortrait = false
            }
            // else : on reste sur l'écran principal, l'app ne se ferme pas
        }
    }
