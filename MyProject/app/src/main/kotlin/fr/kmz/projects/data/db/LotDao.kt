...
@Dao
interface LotDao {
    @Insert suspend fun insert(lot: Lot): Long

    @Update suspend fun update(lot: Lot)

    @Delete suspend fun delete(lot: Lot)

    @Query("DELETE FROM lots") suspend fun clearAllLots()

    @Query("SELECT * FROM lots ORDER BY dateCreation DESC") fun getAllLots(): Flow<List<Lot>>
    ...
    @Query("SELECT * FROM lots WHERE id = :id") suspend fun getLotById(id: Long): Lot?
