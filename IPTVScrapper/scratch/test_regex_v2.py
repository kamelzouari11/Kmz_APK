...
for i, match in enumerate(url_matches):
    ...
    # NEW PATTERNS
    user_pattern = re.compile(r"(?:Username|User|Utilisateur|Login|Account|👤Username|👤User|USER|👤)\s*[:\u27A4]?\s*([^\s\"'<>]+)", re.IGNORECASE)
    pass_pattern = re.compile(r"(?:Password|Pass|Pwd|Mot\s*de\s*passe|Motdepasse|🔑Password|🔑Pass|Password🔑|PASS|🔑)\s*[:\u27A4]?\s*([^\s\"'<>]+)", re.IGNORECASE)
