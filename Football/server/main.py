...
MAX_DAILY_SCHEDULE_MATCHES = 500
LOGGER = logging.getLogger("football-tv-cache")

# Fixed lock stripes keep coordination bounded even with arbitrary requested dates.
_CACHE_LOCKS = [RLock() for _ in range(64)]
detail_cache = OrderedDict()
DETAIL_CACHE_SECONDS = 60
DETAIL_CACHE_LIMIT = 512
_DETAIL_CACHE_LOCK = RLock()


def serialized_by_key(key_for):
    """Coalesce concurrent refreshes; unrelated keys can still progress."""
...
@serialized_by_key(lambda match: ("detail", match.get("sourceUrl"), match.get("date")))
def get_match_detail_cached(match: Dict) -> Dict:
    key = (match.get("date"), match.get("sourceUrl"))
    with _DETAIL_CACHE_LOCK:
        entry = detail_cache.get(key)
        if entry is not None and entry["expiry"] > monotonic():
            detail_cache.move_to_end(key)
            return {**match, **entry["data"]}
    try:
        enriched = enrich_live_soccertv_match(match)
        # Only cache detail fields, never the fixture's team order or schedule.
        data = {field: enriched[field] for field in (
            "channels", "channelCountries", "events", "eventsSnapshot",
            "source", "sourceUrl", "verifiedAt"
        ) if field in enriched}
    except Exception as exc:
        LOGGER.warning("Match detail refresh failed for %s: %s", key, exc)
        data = entry["data"] if entry is not None else {}
    with _DETAIL_CACHE_LOCK:
        detail_cache[key] = {
            "expiry": monotonic() + DETAIL_CACHE_SECONDS,
            "data": data
        }
        detail_cache.move_to_end(key)
...
def deduplicate_matches(matches: List[Dict]) -> List[Dict]:
    ...
    for match in matches:
        teams = sorted((
            team_identity_key(match.get("home", "")),
            team_identity_key(match.get("away", ""))
        ))
        key = (
            match.get("date"),
            teams[0],
...
def parse_liveonsat_date(date_str: str, current_year: int = None) -> Optional[str]:
    ...
    year_hint_supplied = current_year is not None
    if not current_year:
    ...
    for fmt in formats:
        try:
            ...
            if not has_year and not year_hint_supplied:
                today = datetime.now().date()
                candidates = []
                for candidate_year in (today.year - 1, today.year, today.year + 1):
                    try:
                        candidates.append(dt.replace(year=candidate_year))
                    except ValueError:
                        continue
                if candidates:
                    dt = min(
                        candidates,
                        key=lambda candidate: abs((candidate.date() - today).days)
                    )
...
def scrape_live_soccertv(target_date: Optional[str] = None) -> List[Dict]:
    ...
    used_full_page_fallback = getattr(response, "status_code", None) == 422
    if used_full_page_fallback:
        fallback_headers = {
            "X-Respond-With": "markdown",
    ...
    matches = []
    # The selected daily table does not contain a date heading, so the requested
    # date is authoritative there. A full-page fallback can contain many days:
    # only explicit headings may assign a date in that response.
    current_date = None if used_full_page_fallback else target_date
    ...
    if target_date:
        target_match_count = sum(
            match.get("date") == target_date for match in matches
        )
        if target_match_count > MAX_DAILY_SCHEDULE_MATCHES:
            raise Exception(
                "Rejected implausible LiveSoccerTV daily schedule: "
                f"{target_match_count} matches for {target_date}."
            )
    return matches
...
def normalize_team_name(name: str) -> set:
    """Return stable identity tokens without discarding meaningful club words."""
    name = name.casefold()
    ...
    # Provider aliases are resolved before legal club markers are removed.
    name = {
        ...
        "club sportif sfaxien": "sfaxien",
        "cs sfaxien": "sfaxien",
        "seville": "sevilla",
        "man utd": "manchester united",
    }.get(name, name)
    # Remove only organizational markers. Words such as United, City, Real and
    # Athletic are meaningful: deleting them made unrelated clubs match.
    common_terms = {
        "fc", "sc", "cf", "fk", "afc", "ac", "as", "cs",
        "club", "sportif", "sportive"
    }
...
def team_identity_key(name: str) -> str:
    """Deterministic key used for cache identity and unordered fixture pairs."""
    tokens = normalize_team_name(name)
    if tokens:
        return " ".join(sorted(tokens))
    return " ".join(name.casefold().split())

def match_teams(request_team: str, scraped_team: str) -> bool:
    """Match provider variants while rejecting one-word accidental overlaps."""
    ...
    if req_words == scr_words:
        return True

    overlap = req_words.intersection(scr_words)
    # Accept longer official variants only when at least two identity words agree.
    # A single shared word (Manchester, Real, United...) is never sufficient.
    if len(overlap) >= 2 and (
        req_words.issubset(scr_words) or scr_words.issubset(req_words)
    ):
        return True
...
def tv_retry_key(home: str, away: str, date: str) -> str:
    teams = sorted([
        team_identity_key(home),
        team_identity_key(away)
    ])
...
def scrape_schedule_for_date(target_date: str) -> List[Dict]:
    """Return the requested day's LiveSoccerTV schedule."""
    matches: List[Dict] = []
    errors: List[str] = []
...
def scrape_match_for_date(
    ...
    """Find the requested fixture in LiveSoccerTV."""
    matches: List[Dict] = []
    errors: List[str] = []
    scrapers: List[Callable[[], List[Dict]]] = [
        lambda: scrape_live_soccertv(target_date)
...
@serialized_by_key(lambda target_date: ("schedule", target_date))
def get_matches_cached(target_date: str) -> List[Dict]:
    ...
    try:
        ...
        if entry is not None:
            # Do not hammer an unavailable upstream on every incoming request.
            entry["expiry"] = datetime.now() + EMPTY_CACHE_DURATION
            return entry["data"]
        raise
...
def get_tv_channels(
    ...
    # A known match page contains the detailed coverage and events. Read it before
    # considering a full schedule refresh: reloading the whole day first adds a
    # second upstream request to the common detail-screen path.
    ...
    if target_match is not None and target_match.get("sourceUrl"):
        target_match = get_match_detail_cached(target_match)

    # Refresh the daily schedule only when it can supply a missing fixture/link.
    # Empty results are throttled independently per match.
    retry_key = tv_retry_key(home, away, date)
    retry_allowed_at = empty_tv_retry_after.get(retry_key)
    if (
        (
            target_match is None or
            (
                not target_match.get("sourceUrl") and
                not target_match.get("channels")
            )
        ) and
